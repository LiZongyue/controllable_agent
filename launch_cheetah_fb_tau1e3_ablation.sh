...
RUN_TAG="${RUN_TAG:-20260819_cheetah_fb_tau1e3_ablation}"
...
for index in "${!VARIANTS[@]}"; do
  ...
  run_name="${variant}_cheetah_run_seed${SEED}_tau1e3"
  session="cfbtaulow_${variant}_cheetah_run"
...
write_job() {
...

  {
    ...
    printf 'echo "[start] $(date -u +%%FT%%TZ) run=%s gpu=%s dino_frame_stack=%s actor_lr=0.0001 fb_lr=0.0001 tau=0.001 auto_resume=false load_model=null load_replay_buffer=null" | tee "$run_dir/launcher.log"\n' \
