...
# Four-task B-side FLARE DINO CLS3 Cheetah runs with every active LR at 1e-5.
...
CAMPAIGN="${CAMPAIGN:-20260904_cheetah_cls3_flareB_alllr1e5_s1}"
...
usage() {
  cat <<'EOF'
Usage: ./launch_cheetah_flare_b_alllr1e5.sh [--dry-run] [--campaign NAME]

Launches the four requested B-side FLARE, pure-visual Cheetah runs.
...
if [[ "${#GPUS_ARRAY[@]}" -ne 4 ]]; then
  echo "GPUS must contain exactly four ordinals." >&2
  exit 2
fi
...
RUN_NAMES=(
  dino_cls3_cheetah_walk_seed1_flareB_alllr1e5_ortho1
  dino_cls3_cheetah_run_seed1_flareB_alllr1e5_ortho1
  dino_cls3_cheetah_run_backward_seed1_flareB_alllr1e5_ortho1
  dino_cls3_cheetah_walk_backward_seed1_flareB_alllr1e5_ortho1
)
SESSIONS=(
  cflare1e5_cheetah_walk_s1
  cflare1e5_cheetah_run_s1
  cflare1e5_cheetah_run_backward_s1
  cflare1e5_cheetah_walk_backward_s1
)
...
EOF
    printf 'echo "[start] $(date -u +%%FT%%TZ) run=%s gpu=%s dino_flare_b=true cls_frames=3 agent_lr=%s fb_lr=%s lr_f=%s lr_b=%s lr_actor=%s ortho=%s" | tee "$run_dir/launcher.log"\n' \
...
printf 'task\tgpu\tsession\tadapter_mode\trun_name\trun_dir\tcheckpoint_dir\tstdout_log\twandb_entity\twandb_project\twandb_run_id\twandb_url\tnum_train_frames\teval_every_frames\tsnapshot_at\tdino_frame_stack\tagent_lr\tfb_lr\tlr_f\tlr_b\tlr_actor\tortho_coef\tfb_target_tau\tz_dim\tmix_ratio\tbatch_size\tdino_separate_fb_adapters\tdino_separate_backward_adapter\tdino_flare_b\tsource_fingerprint\tjob_file\n' > "$manifest"
for index in "${!TASKS[@]}"; do
  ...
  manifest_row=(
    "${TASKS[$index]}" "${GPUS_ARRAY[$index]}" "${SESSIONS[$index]}" "flare_b"
    ...
    "false" "false" "true" "$SOURCE_FINGERPRINT" "$job_file"
  )
  (
    IFS=$'\t'
    printf '%s\n' "${manifest_row[*]}"
...
echo "All four B-side FLARE all-LR=1e-5 runs launched."
