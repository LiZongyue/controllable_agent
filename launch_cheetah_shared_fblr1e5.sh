...
# Four-task shared-adapter DINO CLS3 Cheetah control at fb_lr=1e-5.
...
CAMPAIGN="${CAMPAIGN:-20260830_cheetah_cls3_shared_fblr1e5_s1}"
...
usage() {
  cat <<'EOF'
Usage: ./launch_cheetah_shared_fblr1e5.sh [--dry-run] [--campaign NAME]

Launches the four requested shared-adapter, pure-visual Cheetah runs.
...
RUN_NAMES=(
  dino_cls3_cheetah_walk_seed1_fblr1e5_ortho1
  dino_cls3_cheetah_run_seed1_fblr1e5_ortho1
  dino_cls3_cheetah_run_backward_seed1_fblr1e5_ortho1
  dino_cls3_cheetah_walk_backward_seed1_fblr1e5_ortho1
)
SESSIONS=(
  cshr1e5_cheetah_walk_s1
  cshr1e5_cheetah_run_s1
  cshr1e5_cheetah_run_backward_s1
  cshr1e5_cheetah_walk_backward_s1
)
...
EOF
    printf 'echo "[start] $(date -u +%%FT%%TZ) run=%s gpu=%s shared_adapter=true cls_frames=3 fb_lr=%s ortho=%s" | tee "$run_dir/launcher.log"\n' \
      "$run_name" "$gpu" "$FB_LR" "$ORTHO_COEF"
...
printf 'task\tgpu\tsession\tadapter_mode\trun_name\trun_dir\tcheckpoint_dir\tstdout_log\twandb_entity\twandb_project\twandb_run_id\twandb_url\tnum_train_frames\teval_every_frames\tsnapshot_at\tdino_frame_stack\tagent_lr\tfb_lr\tortho_coef\tfb_target_tau\tz_dim\tmix_ratio\tbatch_size\tdino_separate_fb_adapters\tdino_separate_backward_adapter\tsource_fingerprint\tjob_file\n' > "$manifest"
for index in "${!TASKS[@]}"; do
  ...
  printf '%s\t%s\t%s\tshared\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tfalse\tfalse\t%s\t%s\n' \
    ...
    "$SNAPSHOT_AT" "$DINO_FRAME_STACK" "$AGENT_LR" "$FB_LR" "$ORTHO_COEF" \
    "$FB_TARGET_TAU" "$Z_DIM" "$MIX_RATIO" "$BATCH_SIZE" "$SOURCE_FINGERPRINT" \
    "$job_file" >> "$manifest"
done
...
echo "All four shared-adapter fb_lr=1e-5 runs launched."
