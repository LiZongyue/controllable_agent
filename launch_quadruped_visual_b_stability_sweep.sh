...
TIMESTAMP="${TIMESTAMP:-$(date -u +%Y%m%d_%H%M%S)_quadruped_visual_b_stability}"
DRY_RUN=0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage: ./launch_quadruped_visual_b_stability_sweep.sh [--dry-run] [--timestamp NAME]

Launch a matched, single-frame visual-backward-map Quadruped stability sweep.
All arms use the same seed and differ in exactly one FB stability knob.
...
# Pin every baseline knob in every arm. The suffix changes exactly one value.
VARIANTS=(
  "control|agent.backward_encoder_grad_scale=1.0 agent.ortho_coef=1.0 agent.mix_ratio=0.5 agent.fb_target_tau=0.01 agent.lr_coef=1.0"
  "slow_target|agent.backward_encoder_grad_scale=1.0 agent.ortho_coef=1.0 agent.mix_ratio=0.5 agent.fb_target_tau=0.001 agent.lr_coef=1.0"
  "no_b_z_feedback|agent.backward_encoder_grad_scale=1.0 agent.ortho_coef=1.0 agent.mix_ratio=0.0 agent.fb_target_tau=0.01 agent.lr_coef=1.0"
  "slow_b|agent.backward_encoder_grad_scale=1.0 agent.ortho_coef=1.0 agent.mix_ratio=0.5 agent.fb_target_tau=0.01 agent.lr_coef=0.1"
  "weak_orth|agent.backward_encoder_grad_scale=1.0 agent.ortho_coef=0.1 agent.mix_ratio=0.5 agent.fb_target_tau=0.01 agent.lr_coef=1.0"
  "mild_b_projector_grad|agent.backward_encoder_grad_scale=0.1 agent.ortho_coef=1.0 agent.mix_ratio=0.5 agent.fb_target_tau=0.01 agent.lr_coef=1.0"
)
...
for spec in "${VARIANTS[@]}"; do
  ...
  session="qvb_${TIMESTAMP}_s${SEED}_${variant}_g${GPU}"
...

  {
    ...
    printf '%q ' \
      ...
      "final_tests=0" \
      "experiment=quadruped_visual_b_stability_${variant}_seed${SEED}" \
      "task=quadruped_walk" \
      "seed=$SEED" \
